<?php
	...
	namespace Core\Classes\Database\Connect\PDO;
	...
	class MySQL{
		...
		public function __construct(){
			...
			$this->params = $this->config->database[$this->config->core['db_driver']];

			try{
				$this->pdo = new PDO(
					"{$this->config->core['db_driver']}:host={$this->params['host']};port={$this->params['port']};",
					$this->params['user'],
					$this->params['pass']
				);
		...
		private function setCharset(){
			$this->pdo->query("SET NAMES {$this->params['sql_charset']};");
			$this->pdo->query("SET CHARACTER SET {$this->params['sql_charset']};");
			return $this;
		}
		...
		public function selectDB($database_name){
			...
			if(fx_is_cli()){
				$this->makeDb($this->params['base'],$this->params['sql_charset'],$this->params['collate']);
			}
			$this->exec("USE {$this->params['base']};");
			return $this;
		}
		 ...
		 * @return array
		 */
		public function getAll($result,$resulttype=PDO::FETCH_ASSOC){
			if($result){
				$data = $result->fetch($resulttype);
