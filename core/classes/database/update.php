<?php
	...
	/*
		Database::update('controllers')
			...
			->data('USERS_ID',5)
//			->limit(1)
			->join('users FORCE INDEX(PRIMARY)','users_id=controllers_id')
			->join('auth FORCE INDEX(PRIMARY)','auth_id=users_id')
			->join('photos FORCE INDEX(PRIMARY)','auth_id=photos_id')
			->join('status FORCE INDEX(PRIMARY)','photos_id=status_id')
			->offset(0)
			->getRows()
	*/
