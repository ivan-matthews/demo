	...
	class Console{
		...
		public function getAliasesFile(){
			if(!$this->aliases_file_data){
				$this->aliases_file_data = fx_load_helper("system/additional/console_commands_aliases");
			}
			return $this;
		}
		...
		public function callHelpCenter(){
			Paint::exec(function(Paint $paint){
				...
				$paint->string($delimiter)->toPaint()->eol()
					...
					->string($delimiter)->toPaint()->eol()
				;
			});
			$this->getArguments(array('cli','help'));
