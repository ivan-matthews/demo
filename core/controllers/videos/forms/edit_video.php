	...
	class Edit_Video extends Form{
		...
		public function generateFieldsList(){		// для метода GET - генерирует поля
			$this->validator_interface->form(function(FormInterface $form){
				...
				$form->setFormAction(fx_get_url('videos','edit',$this->video_id));
			});
			$this->validator_interface->field('v_name')
				->htmlspecialchars(true)
				->id('name')
				->title('title')
				->type('text')
				->placeholder(fx_lang('videos.placeholder_name'))
				->label(fx_lang('videos.label_name'))
			...
			$this->validator_interface->field('v_description')
				->jevix(true)
				->id('description')
				->title('title')
				->type('textarea')
				->placeholder(fx_lang('videos.placeholder_description'))
				->label(fx_lang('videos.label_description'))
				->params(function(Params $params){
