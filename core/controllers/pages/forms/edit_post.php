<?php
	...
	/*
		-------GET-------
	 		$form = new \Core\Controllers\Pages\Forms\Simple('form');
			$form->generateFieldsList();
			fx_die($form->getFieldsList());
		-------GET-------

		-------POST-------
			$form = new \Core\Controllers\Pages\Forms\Simple();
	...
	*/
	...
	namespace Core\Controllers\Pages\Forms;
	...
	class Edit_Post extends Form{
		...
		public function generateFieldsList($post_id){		// для метода GET - генерирует поля
			...
			$this->validator_interface->form(function(FormInterface $form){
				...
				$form->setFormAction(fx_get_url('pages','edit',$this->post_id));
			});

			$this->validator_interface->field('pg_title')
				->htmlspecialchars()
//				->class('class')
				->id('title')
				->label(fx_lang('pages.title_field'))
				->placeholder(fx_lang('pages.title_field_placeholder'))
				->type('text')
				->check(function(Checkers $checkers){
					$checkers->max(191);
				});

			$this->validator_interface->field('pg_image_preview_id')
				->htmlspecialchars()
//				->class('class')
				->id('image-field')
				->label(fx_lang('pages.add_image_to_form'))
				->type('hidden')
				->autocomplete('off')
			...
			$this->validator_interface->field('pg_content')
				->jevix(true)
//				->class('class')
				->id('content')
				->label(fx_lang('pages.content_field'))
				->placeholder(fx_lang('pages.content_field_placeholder'))
				->type('post')
				->params(function(Params $param){
...
//				->label(fx_lang('attachments.attachments_field_label'))
			...
			$this->validator_interface->field('pg_category_id')
				->prepare()
				->id('title')
...
//					$params->default_value($this->current_category);
			...
			$this->validator_interface->field("pg_comments_enabled")
				->prepare()
				->params(function(Params $params){
					...
					$params->default_value(isset($this->data['pg_comments_enabled']) ? $this->data['pg_comments_enabled'] : true);
				})
				->title(fx_lang('pages.comments_enabled'))
				->label(fx_lang('pages.comments_enabled'))
				->type('checkbox')
//				->class('simple')
				->id('enable_comments');

			$this->validator_interface->field("pg_public")
				->prepare()
				->params(function(Params $params){
					...
					$params->show_validation(false);
					$params->field_sets_field_class('ml-2');
					$params->default_value(isset($this->data['pg_public']) ? $this->data['pg_public'] : true);
				})
				->title(fx_lang('pages.item_is_public'))
				->label(fx_lang('pages.item_is_public'))
				->type('checkbox')
//				->class('simple')
				->id('public');
		...
		public function checkCategory(){
			$category = $this->getValue('pg_category_id');
