<?php
	...
	/*
		-------GET-------
	 		$form = new \Core\Controllers\Redirect\Forms\Simple('form');
			$form->generateFieldsList();
			fx_die($form->getFieldsList());
		-------GET-------

		-------POST-------
			$form = new \Core\Controllers\Redirect\Forms\Simple();
	...
	*/
	...
	namespace Core\Controllers\Redirect\Forms;
	...
	class Simple extends Form{
		...
		public function generateFieldsList(){		// для метода GET - генерирует поля
			$this->validator_interface->form(function(FormInterface $form){
				...
				$form->setFormAction(fx_get_url('redirect','index'));
			});
