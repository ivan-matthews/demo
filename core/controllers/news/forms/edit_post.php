<?php
	...
	/*
		-------GET-------
	 		$form = new \Core\Controllers\News\Forms\Simple('form');
			$form->generateFieldsList();
			fx_die($form->getFieldsList());
		-------GET-------

		-------POST-------
			$form = new \Core\Controllers\News\Forms\Simple();
	...
	*/
	...
	namespace Core\Controllers\News\Forms;
	...
	class Edit_Post extends Form{
		...
		public function generateFieldsList($post_id){		// для метода GET - генерирует поля
			...
			$this->validator_interface->form(function(FormInterface $form){
				...
				$form->setFormAction(fx_get_url('news','edit',$this->post_id));
			});

			$this->validator_interface->field('nw_title')
				->htmlspecialchars()
//				->class('class')
				->id('title')
				->label(fx_lang('news.title_field'))
				->placeholder(fx_lang('news.title_field_placeholder'))
				->type('text')
				->check(function(Checkers $checkers){
					$checkers->max(191);
				});

			$this->validator_interface->field('nw_image_preview_id')
				->htmlspecialchars()
//				->class('class')
				->id('image-field')
				->label(fx_lang('news.add_image_to_form'))
				->type('hidden')
				->autocomplete('off')
			...
			$this->validator_interface->field('nw_content')
				->jevix(true)
//				->class('class')
				->id('content')
				->label(fx_lang('news.content_field'))
				->placeholder(fx_lang('news.content_field_placeholder'))
				->type('post')
				->params(function(Params $param){
...
//				->label(fx_lang('attachments.attachments_field_label'))
			...
			$this->validator_interface->field('nw_category_id')
				->prepare()
				->id('title')
...
//					$params->default_value($this->current_category);
			...
			$this->validator_interface->field("nw_comments_enabled")
				->prepare()
				->params(function(Params $params){
					...
					$params->default_value(isset($this->data['nw_comments_enabled']) ? $this->data['nw_comments_enabled'] : true);
				})
				->title(fx_lang('news.comments_enabled'))
				->label(fx_lang('news.comments_enabled'))
				->type('checkbox')
//				->class('simple')
				->id('enable_comments');

			$this->validator_interface->field("nw_public")
				->prepare()
				->params(function(Params $params){
					...
					$params->show_validation(false);
					$params->field_sets_field_class('ml-2');
					$params->default_value(isset($this->data['nw_public']) ? $this->data['nw_public'] : true);
				})
				->title(fx_lang('news.item_is_public'))
				->label(fx_lang('news.item_is_public'))
				->type('checkbox')
//				->class('simple')
				->id('public');
		...
		public function checkCategory(){
			$category = $this->getValue('nw_category_id');
